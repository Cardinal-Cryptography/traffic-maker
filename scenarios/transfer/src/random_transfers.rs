...
use chain_support::{keypair_derived_from_seed, try_send_xt, with_event_listening, Event};
...
impl RandomTransfers {
    ...
    async fn send_sequentially(
        ...
        for (idx, transfer_pair) in pairs.into_iter().enumerate() {
            ...
            let transfer_result = try_transfer(connection, &sender, &receiver, 1).await;
    ...
    async fn send_in_batch(
        ...
        for transfer_pair in pairs.clone() {
            ...
            let metadata = connection.clone().set_signer(sender).metadata;
        ...
        let connection = connection.clone().set_signer(pairs[0].sender.clone());
